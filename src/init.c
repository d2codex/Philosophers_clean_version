...
/*   Updated: 2025/08/25 17:32:56 by diade-so         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
/**
 * @brief Initialize and validate philosopher simulation arguments.
 *
 * Checks argument count, validates values via validate_args(),
 * and sets defaults when necessary.
 *
...
int	init_args(int ac, char **av, t_args *args)
{
	if (ac < 5 || ac > 6)
	{
		error_return("Invalid number of arguments\n");
		print_usage();
		return (1);
	}
	if (validate_args(ac, av, args))
	{
		print_usage();
		return (1);
	}
